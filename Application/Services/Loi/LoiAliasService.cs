...
namespace Application.Services.Loi
{
    public sealed class LoiAliasService : ILoiAliasService
    {
        ...
        {
            await RequireProjectManagerAsync(projectId, actor, isSystemAdmin, ct);

            var aliases = await _rules.GetAliasesForProjectAsync(projectId, ct);
            ...
            Guid projectId, CreateLoiAliasDTO dto, Guid actor, bool isSystemAdmin, CancellationToken ct = default)
        {
            ...
            var isKnownParam = await _rules.RequirementParamExistsAsync(standard, ct);
            if (!isKnownParam)
                throw new ApiExceptionResponse(
                    $"\"{dto.StandardParamName}\" không phải tham số chuẩn trong bảng BXD 347 Phụ lục 02.", 400);

            var existing = await _rules.FindAliasAsync(alias, projectId, ct);
            ...
            Guid projectId, Guid aliasId, Guid actor, bool isSystemAdmin, CancellationToken ct = default)
        {
            ...
            var alias = await _rules.GetAliasForUpdateAsync(aliasId, ct)
        ...
        private async Task<Project> RequireProjectAsync(Guid projectId, CancellationToken ct) =>
            await _rules.GetProjectAsync(projectId, ct)
            ?? throw new ApiExceptionResponse("Project not found.", 404);

        private async Task RequireProjectManagerAsync(
