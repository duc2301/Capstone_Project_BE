...
namespace Capstone_Project.Controllers
{
    ...
    {
        private readonly IFolderTemplateService _service;

        public FolderTemplatesController(IFolderTemplateService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CreateFolderTemplateDTO dto)
            => Ok(ApiResponse.Success("Created successfully", await _service.CreateAsync(dto)));

        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdateFolderTemplateDTO dto)
