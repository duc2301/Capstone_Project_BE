...
namespace Capstone_Project.Controllers
{
    ...
    public class MarkupsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateSet([FromBody] CreateMarkupSetDTO dto, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup set created", await _service.CreateSetAsync(dto, User.GetAccountId(), User.IsAdmin(), ct)));

        [HttpGet("sets/by-file/{fileItemId:guid}")]
        public async Task<IActionResult> GetByFile(Guid fileItemId, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup sets retrieved", await _service.GetSetsByFileAsync(fileItemId, User.GetAccountId(), User.IsAdmin(), ct)));

        [HttpGet("sets/by-issue/{issueId:guid}")]
        public async Task<IActionResult> GetByIssue(Guid issueId, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup sets retrieved", await _service.GetSetsByIssueAsync(issueId, User.GetAccountId(), User.IsAdmin(), ct)));

        [HttpGet("sets/{setId:guid}")]
        public async Task<IActionResult> GetSetDetail(Guid setId, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup set retrieved", await _service.GetSetDetailAsync(setId, User.GetAccountId(), User.IsAdmin(), ct)));
        ...
        public async Task<IActionResult> UpdateSetStatus(Guid setId, [FromBody] UpdateMarkupSetStatusDTO dto, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup set status updated", await _service.UpdateSetStatusAsync(setId, dto.Status, User.GetAccountId(), User.IsAdmin(), ct)));
        ...
        public async Task<IActionResult> LinkToIssue(Guid setId, [FromBody] LinkMarkupToIssueDTO dto, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup set issue link updated", await _service.LinkToIssueAsync(setId, dto.IssueId, User.GetAccountId(), User.IsAdmin(), ct)));
        ...
        public async Task<IActionResult> AddNote(Guid setId, [FromBody] CreateFileNoteDTO dto, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup note added", await _service.AddNoteAsync(setId, dto, User.GetAccountId(), User.IsAdmin(), ct)));
        ...
        public async Task<IActionResult> UpdateNote(Guid noteId, [FromBody] UpdateFileNoteDTO dto, CancellationToken ct)
            => Ok(ApiResponse.Success("Markup note updated", await _service.UpdateNoteAsync(noteId, dto, User.GetAccountId(), User.IsAdmin(), ct)));

        [HttpDelete("notes/{noteId:guid}")]
        public async Task<IActionResult> DeleteNote(Guid noteId, CancellationToken ct)
        {
            await _service.DeleteNoteAsync(noteId, User.GetAccountId(), User.IsAdmin(), ct);
