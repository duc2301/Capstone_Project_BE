...
namespace Infrastructure.Repositories
{
    public class FolderPermissionRepository : GenericRepository<FolderPermission>, IFolderPermissionRepository
    {
        ...
        /// <summary>
        /// Get the ProjectParticipant ids (within the folder's project) that the caller belongs to,
        ...
        /// </summary>
        public async Task<HashSet<Guid>> GetCallerParticipantIdsByFolderIdAsync(Guid folderId, Guid accountId)
        {
            var projectId = await _context.Folders
                            .Where(f => f.Id == folderId)
                            .Select(f => f.ProjectId)
